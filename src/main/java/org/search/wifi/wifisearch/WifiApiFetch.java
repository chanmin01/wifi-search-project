package org.search.wifi.wifisearch;
...
public class WifiApiFetch {

    public static ArrayList<WifiData> loadWifiObjectList() throws IOException {
        ArrayList<WifiData> wifiData = new ArrayList<>();
        String baseUrlString = "http://openapi.seoul.go.kr:8088/706965665263686135357951564f6d/json/TbPublicWifiInfo";
...
//        System.out.println(jsonArray);
    ...
    public static ArrayList<WifiData> jsonToWifiData(String jsonRawString) {
        ...
        try {
            ...
            JsonElement jsonElement = JsonParser.parseString(jsonRawString);

            // JSON이 객체인지 확인
            if (jsonElement.isJsonObject()) {
                JsonObject jsonObject = jsonElement.getAsJsonObject();

                // "TbPublicWifiInfo" 객체가 존재하는지 확인
                if (jsonObject.has("TbPublicWifiInfo") && jsonObject.get("TbPublicWifiInfo").isJsonObject()) {
                    JsonObject element = jsonObject.getAsJsonObject("TbPublicWifiInfo");

                    // "row" 배열이 존재하는지 확인
                    if (element.has("row") && element.get("row").isJsonArray()) {
                        JsonArray jsonArray = element.getAsJsonArray("row");

                        // 배열 내의 각 요소를 WifiData 객체로 변환
                        for (JsonElement item : jsonArray) {
                            WifiData wifiData = gson.fromJson(item, WifiData.class);
                            wifiDataArrayList.add(wifiData);
                        }
                    } else {
                        System.out.println("The 'row' field is missing or is not an array.");
                    }
                } else {
                    System.out.println("The 'TbPublicWifiInfo' field is missing or is not an object.");
                }
            } else {
                System.out.println("Expected a JsonObject but got a different type.");
            }
